...
import { abi } from '../abi';
import LazyMinter from '../voucherCreate';
const ethers = require("ethers");

const web3 = new Web3('https://goerli.infura.io/v3/9aa3d95b3bc440fa88ea12eaa4456161');
const ethersProvider = new ethers.ethers.JsonRpcProvider('https://goerli.infura.io/v3/9aa3d95b3bc440fa88ea12eaa4456161');
...
export class RoomBattle extends RoomGames.RoomGame<RoomBattlePlayer> {
	...
	receive(lines: string[]) {
		...
		switch (lines[0]) {
			...
			case 'end':
				...
				if (!this.ended) {
					this.ended = true;
					void this.onEnd(this.logData!.winner, this.players);
					this.clearPlayers();
				}
				this.checkActive();
				break;
	...
	async onEnd(winner: any, players: any | null) {
		this.timer.end();
		...
		console.log('players 1----------------->>>>>>>>>>>>>>>>>>>>>>>>>>>>>', players[0].walletAddress);
		console.log('players 2----------------->>>>>>>>>>>>>>>>>>>>>>>>>>>>>', players[1].walletAddress);

		let winAddress, loseAddress;
		if (winnerid == this.players[0].id) {
			winAddress = players[0].walletAddress;
			loseAddress = players[1].walletAddress;
		} else if (winnerid == this.players[1].id) {
			winAddress = players[1].walletAddress;
			loseAddress = players[0].walletAddress;
		}
		console.log('winnerid == this.players[0].id 1----------------->>>>>>>>>>>>>>>>>>>>>>>>>>>>>', winnerid == this.players[0].id);
		console.log('winnerid == this.players[0].id 2----------------->>>>>>>>>>>>>>>>>>>>>>>>>>>>>', winnerid == this.players[0].id);
		console.log('winAddress 1----------------->>>>>>>>>>>>>>>>>>>>>>>>>>>>>', winAddress);
		console.log('loseAddress 2----------------->>>>>>>>>>>>>>>>>>>>>>>>>>>>>', loseAddress);
		// console.log('this.p1 ----------------->>>>>>>>>>>>>>>>>>>>>>>>>>>>>', this.p1);
		// console.log('this.p2 ----------------->>>>>>>>>>>>>>>>>>>>>>>>>>>>>', this.p2);


		// const privateKey = '7e4b1bb5bd459ca095bbcd10f8971f82d325366494af3087209fb3e659a3ce59';
		// console.log('privateKey', privateKey)
		// const signer = new ethers.Wallet(privateKey, ethersProvider);
		// console.log('signer', signer)

		// const contractAddress = '0x62dfe198C6B5Aad15AFE5fE3ae073ea8a8D5f869';
		...
		// let voucher = await lazyMinter.createBattle(players[0].walletAddress, players[1].walletAddress);

		// console.log('voucher ----------->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>', voucher);

		// const account = web3.eth.accounts.privateKeyToAccount(privateKey);
		// const signer = web3.eth.accounts.sign(account.privateKey);
		...
		// 	from: '0x78cf14d14ce9c31ae11986b848863064dae0d4a2',
		// 	to: contractAddress,
		// 	// gas: 3000000,
		// 	// value: "0x3E8",
		// 	data: contract.methods.endBattleRoom({winner: winAddress, loser: loseAddress, signature:'0x'}).encodeABI(),
		// 	chainId: 5
		//   };
		...
		const wallet = web3.eth.accounts.privateKeyToAccount(
			"7e4b1bb5bd459ca095bbcd10f8971f82d325366494af3087209fb3e659a3ce59"
		);

		console.log('wallet required --->>>>>>>>', wallet.address);

		const nonce = await web3.eth.getTransactionCount(wallet.address, "latest"); // get latest nonce

		// let gas = contract.methods.endBattleRoom({ winner: winAddress, loser: loseAddress, signature: '0x' }).estimateGas((err: any, gasAmount: any) => {
		// 	if (err) {
		// 	  console.error(err);
		// 	  return;
		// 	}

		// 	console.log(`Gas required: ${gasAmount}`);
		//   });

		// Create the transaction
		const tx = {
			from: wallet.address,
			to: '0x62dfe198C6B5Aad15AFE5fE3ae073ea8a8D5f869',
			nonce: nonce,
			data: contract.methods.endBattleRoom({ winner: winAddress, loser: loseAddress, signature: '0x' }).encodeABI(),
			value: 0,
			gas: 3000000
	...
	forfeitPlayer(player: RoomBattlePlayer, message = '') {
		...
		// web3.eth.getBalance(user.walletAddress, (error, balance) => {
		// 	if (error) {
		// 		console.error(error);
		// 	} else {
