...
export const commands: Chat.ChatCommands = {
	...
	trn(target, room, user, connection) {
		...
		const [name, registeredString,walletAddress, token] = Utils.splitFirst(target, ',', 3);
		const registered = !!parseInt(registeredString);

		console.log('name,',' ---', name,' ---',  'registeredString','---',registeredString, 'walletAddress', '---', walletAddress)
		return user.rename(name, token || '', registered, connection, walletAddress);
	},
	trnhelp: [
